...
export const ActionModal: React.FC<ActionModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-[1px] flex items-center justify-center p-2 sm:p-4">
      <div className="bg-white border-2 border-gov-navy rounded-sm max-w-lg w-full max-h-[90vh] overflow-y-auto shadow-lg">
        {/* Modal Header */}
        <div className="bg-gov-navy text-white px-3 sm:px-4 py-2.5 flex items-center justify-between sticky top-0 z-10">
          <div className="flex items-center space-x-2 text-[11px] sm:text-xs font-bold uppercase tracking-wider pr-2">
            {type === 'FREEZE' && <Lock className="w-4 h-4 text-gov-saffron shrink-0" />}
            {type === 'DISPATCH' && <Send className="w-4 h-4 text-gov-saffron shrink-0" />}
            {type === 'CCTV' && <Video className="w-4 h-4 text-gov-saffron shrink-0" />}
            {type === 'DOSSIER' && <FileText className="w-4 h-4 text-gov-saffron shrink-0" />}
            <span>
        ...
        <div className="p-3 sm:p-4 space-y-3">
          {isSuccess ? (
            <div className="py-6 sm:py-8 text-center space-y-2">
              <CheckCircle2 className="w-10 sm:w-12 h-10 sm:h-12 text-gov-green mx-auto" />
              <h3 className="text-sm sm:text-base font-bold text-gov-navy uppercase tracking-wide">
              ...
              <p className="text-xs text-gov-text-muted font-mono px-2 sm:px-4">
            ...
            <>
              {/* Summary Block */}
              <div className="bg-gov-bg p-2.5 sm:p-3 rounded-sm border border-gov-border text-xs space-y-1">
              ...
              {type === 'FREEZE' && (
                <form onSubmit={handleExecute} className="space-y-3 text-xs">
                  ...
                  <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-end gap-2 pt-2 border-t border-gov-border">
                    <button type="button" onClick={onClose} className="gov-btn-secondary px-3 py-2 sm:py-1.5 w-full sm:w-auto">
                      Cancel
                    </button>
                    <button type="submit" className="gov-btn-danger px-4 py-2 sm:py-1.5 font-bold w-full sm:w-auto">
